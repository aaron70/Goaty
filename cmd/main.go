...
func main() {
	...
	templateCompiled := utils.Must(d.CompileTemplate(`{ "id": {{ nextIncrementalInt "id" 0 1}}, "text": "{{ randomText 500 }}" }{{- if ge (currentIncrementalInt "id" 0) 15 -}}{{ fail "bad luck" }}{{ end }}`,
		decoy.WithTemplateNamed("template"),
	))

	_ = concurrency.ToWorkerFunc(func(ctx context.Context, task string) (string, error) {
	...
	pool := utils.Must(concurrency.NewPool(ctx, func(ctx context.Context, task string, sendResult func(string), sendErr func(error)) {
		buffer := new(bytes.Buffer)
		err := templateCompiled.Execute(buffer, "")
		if err != nil {
			sendErr(err)
		} else {
			sendResult(buffer.String())
		}
	},
		// concurrency.NewPoolWithMinWorkers(0),
		// concurrency.NewPoolWithMaxWorkers(250),
		concurrency.NewPoolWithMaxWorkers(runtime.NumCPU()),
		concurrency.NewPoolWithBufferSize(0),
		// concurrency.NewPoolWithKeepAlive(true),
		concurrency.NewPoolWithIdleDuration(time.Millisecond*1500),
	))
...
// 	maxWorkers := 200
//
// 	producerPool := utils.Must(concurrency.NewPool(ctx, func(ctx context.Context, task string) (string, error) {
// 		return decoy.Default.ParseTemplateString(`{ "id": {{ NextIncrementalInt "id" 0 1 }}, "text": "{{ RandomText 100 }}" }`)
// 	},
// 		concurrency.NewPoolWithBufferSize(buffer),
// 		concurrency.NewPoolWithMaxWorkers(maxWorkers),
// 	))
//
// 	consumer1Pool := utils.Must(concurrency.NewPool(ctx, func(ctx context.Context, template string) (string, error) {
...
// 	))
//
// 	consumer2Pool := utils.Must(concurrency.NewPool(ctx, func(ctx context.Context, task string) (string, error) {
// 		time.Sleep(sleep)
// 		return task, nil
// 	},
// 		concurrency.NewPoolWithBufferSize(buffer),
...
// 	))
//
// 	consumer3Pool := utils.Must(concurrency.NewPool(ctx, func(ctx context.Context, task string) (string, error) {
// 		time.Sleep(sleep)
...
// 	done := make(chan struct{})
// 	wg.Go(func() {
// 		defer concurrency.PrintPools([]string{"Producer", "consumer1", "consumer2", "consumer3"}, producerPool, consumer1Pool, consumer2Pool, consumer3Pool)
// 		for {
// 			select {
// 			case <-done:
// 				return
...
// 				return
// 			case <-time.Tick(time.Millisecond * 500):
// 				concurrency.PrintPools([]string{"Producer", "consumer1", "consumer2", "consumer3"}, producerPool, consumer1Pool, consumer2Pool, consumer3Pool)
// 			}
// 		}
// 	})
//
